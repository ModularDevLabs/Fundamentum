...
const WEB3_MODULE_TOOLTIPS = {
  ...
  settingsWeb3WhaleAlertsEnabled: 'Enable whale-buy-pressure signal derived from 24h tx buy/sell mix and estimated buy-side notional.',
  settingsWeb3WhaleMinTradeUsd: 'Minimum estimated 24h buy-side notional (USD) required to trigger whale-buy-pressure signal.',
...
function applyWeb3ModuleTooltips() {
  const root = qs('#view-web3intel');
  if (!root) return;
  Object.entries(WEB3_MODULE_TOOLTIPS).forEach(([id, text]) => {
    const el = root.querySelector(`#${id}`);
    if (!el) return;
    el.title = text;
    const label = el.closest('label');
